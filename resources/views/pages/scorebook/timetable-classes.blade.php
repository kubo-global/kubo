<x-page title="Timetables">
  <div class="px-6 py-6 lg:px-8">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
      ...
      <form method="GET" action="{{ route('timetable.index') }}" class="flex items-center gap-2 text-sm text-gray-500">
    ...
    @else
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        @foreach ($offerings as $offering)
          ...
          @endphp
          <a href="{{ route('timetable.show', $offering) }}"
    ...
    <p class="mt-4 text-xs text-gray-500">
      Opens on the current school year by default. Click a class to view or edit its weekly timetable.
    </p>
  </div>
</x-page>
