<x-page title="Scorebook">
  <div class="px-6 py-6 lg:px-8">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
      ...
      <form method="GET" action="{{ route('reporting.grades') }}" class="flex items-center gap-2 text-sm text-gray-500">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        @foreach ($offerings as $offering)
          ...
          <a href="{{ route('term-grid.report', $offering) }}"
            ...
            <span style="{{ $offering->badgeStyle() }}" class="inline-flex items-center justify-center text-sm font-bold rounded-lg w-12 h-12 shrink-0">{{ $abbr }}</span>
            <div class="min-w-0">
    ...
    <p class="mt-4 text-xs text-gray-500">
      Opens on the current school year by default. Switch the year (top right) only to reach a past year. Then click a class.
    </p>
  </div>
</x-page>
