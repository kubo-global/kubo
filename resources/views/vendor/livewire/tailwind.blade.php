{{--
    Patched copy of livewire::tailwind (vendor/livewire/livewire/.../views/tailwind.blade.php).
    Only change vs upstream: the disabled prev/next <span>s carry role="link" so
    aria-disabled/aria-label are valid (a bare <span> is `generic`, which prohibits
    them — flagged by axe as aria-prohibited-attr). Re-sync on Livewire major upgrades.
--}}
@php
if (! isset($scrollTo)) {
    $scrollTo = 'body';
